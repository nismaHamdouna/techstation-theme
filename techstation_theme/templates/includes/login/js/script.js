jQuery(document).ready(function($) {
	...
	$('.main_container .section_right_inner form .form_group #showPassword').on('click', function(e) {
	 ...
	 $('form').on('submit', function(e) {
        ...
        $(this).find('.form_group_input').each(function() {
            if ($.trim($(this).val()).length == 0){
                $(this).parent('.form_group').removeClass("success");
                $(this).parent('.form_group').addClass("highlight");
                isFormValid = false;
            }
            else{
                $(this).parent('.form_group').removeClass("highlight");
                $(this).parent('.form_group').addClass("success");
            }
        });
        return isFormValid;
    });
    ...
    $('form .form_group_input').on('keyup', function(e) {
    	if ($.trim($(e.target).val()).length !== 0) {
    		$(e.target).parent('.form_group').addClass('success').removeClass('highlight');
    	} else {
    		$(e.target).parent('.form_group').removeClass('success').addClass('highlight');
    	}
    });
