...
def get_context(context):
	...
	last_session = frappe.db.sql('select  user from tabSessions order by lastupdate desc limit 1')[0]

	frappe.local.cookie_manager.init_cookies()
	frappe.local.cookie_manager.set_cookie("first_name", frappe.db.get_value("User", frappe.session.user, "first_name"))

	context['login_name_placeholder'] = ' {0} '.format(_('or')).join(login_name_placeholder)
	#context['last_login']=  frappe.avatar(frappe.session.user)
...
def login_cookies(user):
	last_session = frappe.db.sql('select  user from tabSessions order by lastupdate desc limit  1');
	return last_session

@frappe.whitelist(allow_guest=True)
def validate_mail(user):
	email = frappe.db.sql('select  name from tabUser where name=%s',user);
	if email:
		return email
	else: return 'error'
